...
public class NetItem extends Item {
  ...
  public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
    ...
    if (stack.getCount() == 1) {
      // 只有一个网：直接写入实体 NBT
      stack.set((Supplier) ModDataComponents.ENTITY_HOLDER, nbt);

    } else {
      // 多个网：分出一个并放到玩家物品栏
      ItemStack newStack = stack.split(1);
      newStack.set((Supplier) ModDataComponents.ENTITY_HOLDER, nbt);
...
/*
  ...
  public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
    ...
    public static RegistryAccess registryAccess() {
        if (ServerLifecycleHooks.getCurrentServer() != null)
            return (RegistryAccess)ServerLifecycleHooks.getCurrentServer().registryAccess();
        return ((Level)((Optional<Level>) LogicalSidedProvider.CLIENTWORLD.get(LogicalSide.CLIENT)).orElseThrow()).registryAccess();
    }
    ...
    if (holder.contains("CustomName", Tag.TAG_STRING)) {
      ...
      try {
          return (Component)Component.Serializer.fromJson(s, (HolderLookup.Provider)registryAccess());
  ...
  public Component getName(@Nonnull ItemStack stack) {
    ...
    if (nameComponent != null) {
      ...
      baseName = super.getName(stack);  // 这是默认的名字（如“捕捉网”）
    }
  ...
  public static boolean containsEntity(ItemStack stack) {
    ...
    public static CompoundTag getEntityData(ItemStack stack) {
        return containsEntity(stack) ? (CompoundTag)stack.get((Supplier)ModDataComponents.ENTITY_HOLDER) : new CompoundTag();
    }
  ...
  public static Entity getEntityFromStack(ItemStack stack, Level world, boolean withInfo) {
    return getEntityFromNBT((CompoundTag)stack.get((Supplier)ModDataComponents.ENTITY_HOLDER), world, withInfo);
  }
